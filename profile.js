(() => {
'use strict'; const V=window.SohbetixV30,$=id=>document.getElementById(id);
...
function online(){const on=V.isNickOnline(target);$('profileOnlineV28').textContent=on?'ÇEVRİM İÇİ':'ÇEVRİM DIŞI';$('profileOnlineV28').classList.toggle('offline',!on);}online();setInterval(online,15000);
if(!isOwn){$('profileOwnerNavV28').querySelectorAll('a:not(.active),button').forEach(x=>x.hidden=true);$('profileOwnerActionsV28').hidden=true;}
  ...
  $('profilePhotoInputV28').addEventListener('change',()=>{const f=$('profilePhotoInputV28').files?.[0];if(!f)return;if(f.type==='image/gif'&&!ctx.vip){alert('Hareketli GIF avatarları yalnızca VIP kullanıcılar yükleyebilir.');return;}if(f.size>1200000){alert('Fotoğraf en fazla 1,2 MB olabilir.');return;}const r=new FileReader();r.onload=()=>{const next={...V.getProfileDataByProfile(ctx.profile),photo:String(r.result||'')};V.saveProfileData(ctx.profile.id,next);$('profilePhotoImgV28').src=next.photo;};r.readAsDataURL(f);});
